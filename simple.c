...
#include <string.h>
#include <stdbool.h>
//what else do I need to include or add to the makefile???   getting reference error
void printArray(char arr[5][12]);
void neighborCount(char arr[5][12]);
//char **matrix = parse_life(argv[1]) <--would this be what I use instead of myArray in main??
//would matrix be something I can iterate through in for loops as I did in neighborCount???
char myArray[5][12] = {
  {' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' '},
  {' ',' ', 'X', 'X', ' ','X', 'X', ' ', ' ', 'X', ' ', ' '} ,
  {' ', 'X', 'X', ' ', ' ', 'X', 'X', ' ', ' ', ' ', 'X',' '},
...
void neighborCount(char arr[5][12]) { //would change size of input 
  int i, j;
  char alive = 'X';
  char dead = ' ';
